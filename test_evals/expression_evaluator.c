...
void parse_tokens(const char *expression, TokenArray *tokens) {
    ...
    while (*p) {
        if (isspace(*p)) {
            ...
            token.value.op = *p;
            if (token.type == OPERATOR && token.value.op == '-' && prev_token_type != NUMBER) {
                token_array_push(tokens, (Token) {.type = PAREN, .value.paren = '('});
    ...
    if (open_bracket_count != 0) {
        fprintf(stderr, "Error: Unbalanced brackets in expression\n");
        exit(1);
    }
}
    ...
    for (int i = 0; i < postfix->size; i++) {
        ...
        if (token.type == NUMBER) {
            ...
            switch (token.value.op) {
                ...
                case '/':
                    if (b == 0) {
                        fprintf(stderr, "Attempting to divide by zero (%.2f / %d)\n", a, (int)b);
                        exit(1);
                    }
                    result = a / b;
                    break;
